...
export default function EveningView() {
  ...
  async function loadData() {
    ...
    // Load existing session selections for tomorrow
    ...
    if (session) {
      const { data: tasks } = await supabase
        .from("prep_tasks")
        .select("template_id, quantity_mode")
      ...
      if (tasks) {
        ...
        tasks.forEach((t) => {
          if (t.template_id && t.quantity_mode) map.set(t.template_id, t.quantity_mode);
        });
        setSelections(map);
  ...
  async function saveList() {
    ...
    if (existing) {
      ...
      // Delete old tasks for this session
    ...
    const tasks = Array.from(selections.entries()).map(([templateId, mode]) => {
      ...
      return {
        ...
        kitchen_type: tmpl.kitchen_type,
        quantity_mode: mode,
        done: false,
      };
    });
  ...
  async function addCustomItem() {
    ...
    const { data } = await supabase
      ...
      .insert({
        ...
        full_quantity: newItemFull.trim(),
        half_quantity: newItemHalf.trim(),
        unit: "",
        active: true,
      })
      .select()
      ...
      <div className="text-center">
        ...
        {selections.size > 0 && (
          <p className="text-sm text-gray-500 mt-1">
            Выбрано: <span className="font-semibold text-gray-800">{selections.size}</span> позиц.
          </p>
        )}
      </div>

      {/* Kitchen sections */}
      {([["hot", hotTemplates], ["cold", coldTemplates]] as [KitchenType, PrepItemTemplate[]][]).map(
        ([kitchen, items]) => (
          <div key={kitchen} className="space-y-2">
            ...
            <div className={`rounded-2xl border overflow-hidden divide-y ${
              kitchen === "hot" ? "border-orange-200 divide-orange-100" : "border-blue-200 divide-blue-100"
            }`}>
              {items.map((tmpl) => {
                ...
                return (
                  <div
                    key={tmpl.id}
                    className={`bg-white transition-colors ${selected ? (kitchen === "hot" ? "bg-orange-50" : "bg-blue-50") : ""}`}
                  >
                    {/* Main row */}
                    <button
                    ...
                    >
                      ...
                      {/* Quantity hint */}
                      {!selected && (
                        <span className="text-xs text-gray-400">{tmpl.full_quantity}</span>
                      )}
                    </button>

                    {/* Full / Half toggle — shown only when selected */}
                    {selected && (
                      <div className="flex gap-2 px-4 pb-3">
                        ...
                        >
                          Полный рецепт
                          <span className="block font-normal opacity-80">{tmpl.full_quantity}</span>
                        </button>
                        <button
                        ...
                        >
                          Половина
                          <span className="block font-normal opacity-80">{tmpl.half_quantity}</span>
                        </button>
                      </div>
                    )}
                  </div>
      ...
      <div>
        {!showAddForm ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
            <h3 className="font-semibold text-gray-700 text-sm">Новая позиция</h3>
            <div className="flex gap-2">
              {(["hot", "cold"] as KitchenType[]).map((k) => (
                <button
                  key={k}
                  onClick={() => setNewItemKitchen(k)}
            ...
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Полный рецепт (напр. 2 кг)"
                value={newItemFull}
                onChange={(e) => setNewItemFull(e.target.value)}
                className="flex-1 px-3 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-gray-50"
              />
              <input
                type="text"
                placeholder="Половина (напр. 1 кг)"
                value={newItemHalf}
                onChange={(e) => setNewItemHalf(e.target.value)}
                className="flex-1 px-3 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-gray-50"
              />
            </div>
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!newItemName.trim() || !newItemFull.trim()}
      ...
      {selections.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 shadow-lg">
          <div className="max-w-2xl mx-auto">
            ...
            >
              {saving ? "Сохраняю..." : saved ? "✅ Список сохранён" : `Сохранить список (${selections.size} позиц.)`}
            </button>
          </div>
        </div>
      )}
